...
- Every identity enters at **PENDING** when Kobo submits a provisioning
  request to Nomba.
- On Nomba confirmation, it becomes **ACTIVE** and the virtual account number
  is usable.
- If Nomba rejects provisioning for any reason, it moves to **FAILED** and
...
- An ACTIVE account becomes **LIMITED** when cumulative inflow approaches the
  ...
  avoid disruption. Verify the exact Nomba ceiling values and signaling
  before finalising the `kyc_tier_breached` trigger logic — this is flagged
  as an open item in `docs/NOMBA_INTEGRATION.md`.
...
- A **CLOSED** account can be **ACTIVE** again via
  ...
  does not require a new provisioning call, unless Nomba requires a fresh
  `accountRef` on reopen (see open item #2 in `docs/NOMBA_INTEGRATION.md`).
...
|---------------|------------------------|------------|---------------------------------------------------------------------------------|
| PENDING       | provisioned            | ACTIVE     | `account/service.go` on successful Nomba provisioning response                 |
| PENDING       | provisioning_failed    | FAILED     | `account/service.go` on any non-`"00"` Nomba response or timeout               |
...
|---|---|
| Rename (`PATCH /v1/identities/{id}` with new `display_name`) | `identity_events` row of type `renamed` written; Nomba `accountName` re-submitted. Identity stays in its current state. |
| Metadata update (`PATCH /v1/identities/{id}` with new `metadata`) | `identity_events` row of type `metadata_updated` written. No state change, no Nomba call. |
...
request to Nomba. The virtual account does not yet exist. The identity's
`virtual_account` field in API responses is `null` while in this state.

**What can happen:**
- Nomba confirms provisioning → ACTIVE
- Nomba rejects provisioning, or the Nomba call times out after retries → FAILED
- Inbound transfer arriving while PENDING: should not happen in practice since
  no account number has been issued yet, but if a Nomba-side race produces
...
header on the Nomba provisioning call. If the call fails at the network layer
and Kobo retries, the same idempotency key is reused so Nomba does not create
a second account. See `docs/NOMBA_INTEGRATION.md`.
...
**Key design decision:** Kobo moves an account to LIMITED *before* Nomba
...
This reuses the existing identity ID and, where Nomba allows it, the same
`accountRef`. See open item #2 in `docs/NOMBA_INTEGRATION.md` on whether
Nomba allows the same `accountRef` to be reused or requires a new one.
...
**Meaning:** Nomba rejected the provisioning request, or Kobo's retries were
...
| provisioned          | Successful Nomba provisioning response |
| provisioning_failed  | Failed Nomba provisioning response |
...
confused with the Nomba webhook events Kobo receives internally).
...
- `Transition(current State, event Event) (State, error)` is the single
  ...
  synchronous: no DB calls, no Nomba calls, no logging inside it.
