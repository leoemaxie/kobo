...
- **Live Keys (`kobo_live_`)**: Will hit the production database and trigger real provisioning against Monnify's live systems.
- **Sandbox Keys (`kobo_test_`)**: Will hit the production database but trigger simulated, mock behavior or route to Monnify's sandbox endpoints.
...
## Monnify Webhook Authentication
For inbound events from Monnify, Kobo verifies the cryptographic integrity of the payload using **HMAC-SHA256**. 
Integrators do not need to worry about this. Kobo uses `MONNIFY_WEBHOOK_SECRET` to validate the `X-Monnify-Signature` header, rejecting all spoofed or tampered events automatically.
