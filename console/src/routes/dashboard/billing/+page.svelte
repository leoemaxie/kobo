...
<div class="flex flex-col gap-7">
  <PageHeader title="Billing &amp; Usage">
    {#snippet meta()}
      ...
      <span class="font-inconsolata text-[13px] text-subtle">
        next_invoice: <span class="text-main"
          >{state.billingOverview?.nextInvoiceDate || '2026-11-01'}</span
        >
      </span>
    {/snippet}
    {#snippet actions()}
        ...
        use:enhance={() => {
          return async ({ result, update }) => {
            if (result.type === 'failure')
              toast.error((result.data?.error as string) || 'Failed to initialize checkout');
            else if (result.type === 'error') toast.error('Server error occurred');
            await update();
          };
        }}
      >
        <Button variant="neutral" size="md" type="submit">Manage Payment Method</Button>
      </form>
    {/snippet}
  </PageHeader>
  ...
  <div
    class="grid grid-cols-1 md:grid-cols-2 gap-px bg-border-subtle border border-border-subtle rounded-lg overflow-hidden"
  >
    <CurrentPeriodCard />
    <PlanDetailsCard />
  </div>
