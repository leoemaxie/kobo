...
export const load: PageServerLoad = async ({ locals, setHeaders }) => {
  ...
  const getPct = (cost: number) =>
    totalCost === 0 ? 0 : Math.round((cost / totalCost) * 100);
  ...
  const billingOverview = {
    ...
    usageItems: [
      {
        key: "accounts_provisioned",
        label: "Virtual Accounts",
        calc: `${currentRecord.accountsProvisioned.toLocaleString()} × ₦50`,
        amount: `₦${accountsCost.toLocaleString()}`,
        pct: getPct(accountsCost),
      },
      {
        key: "transaction_fees",
        label: "Transaction Fees",
        calc: `${currentRecord.transactionsProcessed.toLocaleString()} × ₦2`,
        amount: `₦${transactionsCost.toLocaleString()}`,
        pct: getPct(transactionsCost),
      },
      {
        key: "webhook_calls",
        label: "Webhook Deliveries",
        calc: `${currentRecord.webhookDeliveries.toLocaleString()} × ₦0.5`,
        amount: `₦${webhooksCost.toLocaleString()}`,
        pct: getPct(webhooksCost),
      },
    ],
    planDetails: [
