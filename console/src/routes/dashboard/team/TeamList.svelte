      ...
      {#each members as member}
        <tr class="team-row">
          ...
          <td style="padding: 16px 20px; text-align: right; position: relative;">
            <button on:click={() => openDropdownId = openDropdownId === member.id ? null : member.id} style="background: transparent; border: none; color: var(--text-subtle); cursor: pointer; padding: 4px;"><MoreVertical size={16} /></button>
              ...
              <div class="absolute right-6 top-10 w-36 bg-carbon border border-iron rounded-md shadow-lg py-1 z-10" on:mouseleave={() => openDropdownId = null}>
                ...
                <form method="POST" action="?/removeMember" use:enhance={() => {
                  return async ({ result, update }) => {
                    ...
                    else toast.error(result.data?.error || 'Failed to remove');
                    openDropdownId = null;
                    await update();
                  };
                }}>
