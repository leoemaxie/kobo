...
<div class="flex flex-col gap-7">
  ...
  >
    <div class="grid gap-6">
      <CardSection title="Business Information" subtitle="Details about your registered business entity.">
        <div class="grid gap-4 md:grid-cols-2">
          <Input id="businessName" label="Legal Business Name" type="text" name="businessName" required variant="settings" />
          <Input id="regNumber" label="Registration Number (RC)" type="text" name="regNumber" required variant="settings" />
          <div class="md:col-span-2">
            <Input id="address" label="Business Address" type="text" name="address" required variant="settings" />
          </div>
        </div>
      </CardSection>

      <CardSection title="Director Details" subtitle="Primary point of contact for compliance matters.">
        <div class="grid gap-4 md:grid-cols-2">
          <Input id="directorName" label="Full Name" type="text" name="directorName" required variant="settings" />
          <Input id="bvn" label="Bank Verification Number (BVN)" type="text" name="bvn" required variant="settings" />
        </div>
      </CardSection>

      <CardSection title="Document Upload" subtitle="Upload supporting documents. PDF or Image format, max 5MB.">
        <div class="grid gap-4 md:grid-cols-2">
          <label class="border-2 border-dashed border-[var(--border-color)] rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-[var(--bg-element)] transition-colors cursor-pointer group">
            <div class="p-3 bg-[var(--bg-active)] rounded-full mb-3 group-hover:bg-[var(--accent)]/10 transition-colors">
              <FileText class="w-6 h-6 text-muted group-hover:text-[var(--accent)] transition-colors" />
            </div>
          ...
          <label class="border-2 border-dashed border-[var(--border-color)] rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-[var(--bg-element)] transition-colors cursor-pointer group">
            <div class="p-3 bg-[var(--bg-active)] rounded-full mb-3 group-hover:bg-[var(--accent)]/10 transition-colors">
              <UploadCloud class="w-6 h-6 text-muted group-hover:text-[var(--accent)] transition-colors" />
            </div>
