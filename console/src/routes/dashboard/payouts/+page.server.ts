...
export const load: PageServerLoad = async ({ locals, fetch, cookies, setHeaders }) => {
  ...
  try {
    ...
    if (!banksRes.ok) {
      console.error(
        `Failed to fetch banks: ${banksRes.status} ${banksRes.statusText} - ${await banksRes.text()}`,
      );
    }
    const banks = banksRes.ok ? await banksRes.json() : [];

    // Fetch current bank account
    const accountRes = await fetch(`${env.CORE_URL}/console/payouts/bank-account`, { headers });
    if (!accountRes.ok && accountRes.status !== 404) {
      console.error(
        `Failed to fetch bank account: ${accountRes.status} ${accountRes.statusText} - ${await accountRes.text()}`,
      );
    }
    ...
    if (!historyRes.ok) {
      console.error(
        `Failed to fetch payouts: ${historyRes.status} ${historyRes.statusText} - ${await historyRes.text()}`,
      );
    }
...
export const actions: Actions = {
  saveBankAccount: async ({ request, cookies }) => {
    ...
    const lookupRes = await fetch(`${env.CORE_URL}/console/payouts/bank-account/lookup`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    const saveRes = await fetch(`${env.CORE_URL}/console/payouts/bank-account`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
