...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
  <div class="w-full max-w-md bg-element border border-border rounded-xl overflow-hidden shadow-xl">
    <div class="flex items-center justify-between px-6 py-4 border-b border-border bg-sidebar">
      <h2 class="text-sm font-semibold text-main uppercase tracking-widest">Create Standard Key</h2>
    ...
    {#if !secretKey}
      <form method="POST" action="?/createKey" use:enhance={() => {
        ...
        <div>
          ...
          <input id="label" type="text" name="label" placeholder="e.g. Production Main" required 
    ...
    {:else}
      <div class="p-6 flex flex-col gap-5">
        ...
        <div class="pt-2 flex justify-end">
          <button onclick={onClose} class="px-5 py-2 bg-element border border-border text-main text-sm font-bold rounded-lg hover:bg-element-hover transition-colors">
            I've copied the key
          </button>
        </div>
      </div>
