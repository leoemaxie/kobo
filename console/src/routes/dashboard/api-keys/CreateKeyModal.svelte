...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
  <div class="w-full max-w-md bg-carbon border border-iron rounded-xl overflow-hidden shadow-xl">
    <div class="flex items-center justify-between px-6 py-4 border-b border-iron bg-void-black">
      <h2 class="text-sm font-semibold text-paper uppercase tracking-widest">Create Standard Key</h2>
      ...
      <form method="POST" action="?/createKey" use:enhance={() => {
        ...
        return async ({ result, update }) => {
          ...
          if (result.type === 'success' && result.data?.plainSecret) {
            ...
            toast.error(result.data?.error || 'Failed to create key');
          }
        ...
        <div>
          ...
          <input type="text" name="label" placeholder="e.g. Production Main" required 
    ...
    {:else}
      <div class="p-6 flex flex-col gap-5">
        <div class="bg-electric-lime/10 border border-electric-lime/30 rounded-lg p-4">
          ...
          <p class="text-xs text-smoke leading-relaxed">For security reasons, this is the only time it will be shown. If you lose it, you will need to roll the key.</p>
        </div>
        ...
        <div class="pt-2 flex justify-end">
          <button on:click={onClose} class="px-5 py-2 bg-carbon border border-iron text-paper text-sm font-bold rounded-lg hover:bg-graphite transition-colors">
            I've copied the key
          </button>
        </div>
      </div>
