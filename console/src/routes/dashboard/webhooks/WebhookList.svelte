...
<div style="background: var(--bg-element); border: 1px solid var(--border-subtle); border-radius: 8px; overflow: hidden;">
  ...
  <table style="width: 100%; border-collapse: collapse; text-align: left;">
    <thead>
      <tr style="border-bottom: 1px solid var(--border-subtle);">
        ...
        <th style="padding: 12px 20px; font-size: 11px; font-weight: 600; color: var(--text-subtle); text-transform: uppercase; letter-spacing: 0.05em;">Events</th>
        <th style="padding: 12px 20px; font-size: 11px; font-weight: 600; color: var(--text-subtle); text-transform: uppercase; letter-spacing: 0.05em;">Secret</th>
    ...
    <tbody>
      {#each endpoints as ep}
        <tr class="webhook-row">
          <td style="padding: 16px 20px; font-family: monospace; font-size: 13px; color: var(--text-main);">{ep.url}</td>
          <td style="padding: 16px 20px;">
            <div style="display: flex; gap: 6px; flex-wrap: wrap;">
              {#each ep.events as ev}
                <span style="font-family: monospace; font-size: 11px; background: var(--bg-active); border: 1px solid var(--border-color); border-radius: 4px; padding: 2px 6px; color: var(--text-muted);">{ev}</span>
              {/each}
            </div>
          </td>
          <td style="padding: 16px 20px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <code style="font-family: monospace; font-size: 13px; color: {secretRevealed[ep.id] ? 'var(--text-main)' : 'var(--text-muted)'};">
          ...
          <td style="padding: 16px 20px;">
            <div style="display: flex; align-items: center; gap: 6px;">
              <Circle size={8} color={ep.status === 'active' ? 'var(--accent)' : 'var(--text-muted)'} fill={ep.status === 'active' ? 'var(--accent)' : 'var(--text-muted)'} />
