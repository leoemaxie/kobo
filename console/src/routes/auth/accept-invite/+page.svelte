    ...
    {:else}
      <div class="flex flex-col items-center text-center">
        ...
        <p class="text-[14px] text-muted mb-6 leading-relaxed">
          You have been invited to join the <strong class="text-main font-semibold">{data.invite.workspaceName}</strong> workspace as a <strong class="text-main font-semibold capitalize">{data.invite.role}</strong>.
        </p>
        ...
        {#if !data.user}
          <div class="w-full bg-sidebar border border-border rounded-lg p-5 mb-6 text-left">
            ...
            <p class="text-[13px] text-subtle mb-4">You need a Kobo account to accept this invitation.</p>
        ...
        {:else}
          {#if data.isEmailMismatch}
             <div class="w-full bg-red-500/10 border border-red-500/20 rounded-lg p-4 mb-6 text-left">
                <p class="text-[13px] text-red-400">
                  This invitation was sent to a different email address. Please log out and log in with the correct account to accept it.
                </p>
             </div>
          {/if}
          {#if data.user.integratorId}
             <div class="w-full bg-orange-500/10 border border-orange-500/20 rounded-lg p-4 mb-6 text-left">
                <p class="text-[13px] text-orange-400">
                  You already belong to a workspace. You must leave your current workspace in Settings before joining a new one.
                </p>
             </div>
          ...
          {:else}
            <form method="POST" class="w-full" use:enhance={() => {
              ...
              <Button type="submit" variant="primary" size="lg" class="w-full" disabled={loading || data.isEmailMismatch}>
