...
export const load: PageServerLoad = async ({ url, locals }) => {
	...
	const isEmailMismatch = locals.user ? locals.user.email.toLowerCase() !== invite.email.toLowerCase() : false;
...
export const actions: Actions = {
	default: async ({ request, locals }) => {
		...
		if (!user) return fail(401, { error: 'You must be logged in to accept an invitation.' });
		...
		if (user.integratorId) {
			return fail(400, { error: 'You are already in a workspace. Please leave your current workspace before joining a new one.' });
		}
		...
		if (invite.email.toLowerCase() !== user.email.toLowerCase()) {
			return fail(400, { error: 'This invitation was sent to a different email address.' });
		}

		await db.update(invitations).set({ acceptedAt: new Date() }).where(eq(invitations.id, token));
		await db.update(users).set({ integratorId: invite.integratorId, role: invite.role as any }).where(eq(users.id, user.id));
