...
<div class="w-full">
      ...
      use:enhance={() => {
        ...
        return async ({ result, update }) => {
          ...
          if (result.type === 'failure') {
            toast.error(
              (result.data?.error as string) || 'Login failed. Please check your credentials.',
            );
    ...
    >
      ...
      <div class="space-y-1.5">
        <div class="flex items-center justify-between">
          <label
            for="password"
            class="block text-xs font-semibold text-muted uppercase tracking-widest">Password</label
          >
          <a
            href="/auth/forgot-password"
            class="text-xs font-medium text-primary hover:opacity-80 transition-colors"
            >Forgot password?</a
          >
        </div>
        <IconInput
      ...
      <div class="flex items-center gap-3">
        ...
        <label for="remember" class="text-sm text-muted cursor-pointer select-none"
          >Keep me signed in for 7 days</label
        >
      </div>
  ...
  <p class="text-center text-sm text-muted mt-6">
    ...
    <a
      href="/auth/signup"
      class="font-semibold text-primary hover:opacity-80 transition-colors ml-1">Sign up</a
    >
  </p>
</div>
