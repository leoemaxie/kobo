...
export const actions: Actions = {
	resend: async ({ locals, url }) => {
		try {
			...
			if (user.emailVerifiedAt) return fail(400, { error: 'Email is already verified.' });
			...
			EmailService.sendVerificationEmail(user.email, token, url.origin).catch(console.error);
