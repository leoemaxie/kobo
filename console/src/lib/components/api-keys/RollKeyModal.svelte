...
<Modal title="Roll API Key" {onClose}>
    ...
    <form method="POST" action="?/rollKey" use:enhance={() => {
      ...
      <div class="border border-error/30 bg-error-bg rounded-lg p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-main opacity-90 leading-relaxed">Rolling this key will instantly invalidate the current secret. Any systems authenticating with <code class="font-mono text-[10px] bg-background border border-border px-1 py-0.5 rounded text-main">{keyId}</code> will be permanently denied access.</p>
        </div>
      </div>
      ...
      <div class="pt-2 flex justify-end gap-3">
        ...
        <Button type="submit" variant="danger" size="md" disabled={isLoading || confirmText !== 'roll my key'}>
  ...
  {:else}
    <div class="p-6 flex flex-col gap-6">
      <div class="bg-primary-transparent border border-primary-border rounded-lg p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-main opacity-90 leading-relaxed">For security reasons, this is the only time it will be shown. Please copy it and update your environment variables immediately.</p>
        </div>
      </div>
      
      <div class="relative group">
        <input type="text" readonly value={secretKey} class="w-full bg-sidebar border border-border rounded-lg pl-4 pr-12 py-3 text-sm font-mono text-main outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all selection:bg-primary/20" />
        <button onclick={handleCopy} class="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-muted hover:text-primary transition-colors bg-sidebar group-hover:bg-sidebar">
          <Copy size={16} />
        </button>
      </div>
