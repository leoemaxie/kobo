...
<div>
  ...
  >
    <p
      style="font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: var(--text-subtle);"
    >
      Recent Requests
    </p>
    <a
    ...
    "
      >
        {#each ['METHOD', 'PATH', 'STATUS', 'TIME', 'REQ ID', ''] as col}
          <span
            style="font-size: 11px; font-weight: 700; letter-spacing: 0.1em; color: var(--text-muted); text-transform: uppercase;"
          >
            {col}
          </span>
        {/each}
      ...
      "
          onmouseenter={(e) =>
            ((e.currentTarget as HTMLDivElement).style.background = 'var(--bg-element)')}
          onmouseleave={(e) =>
