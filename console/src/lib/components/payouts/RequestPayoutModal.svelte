<script lang="ts">
	...
	let { isOpen, onClose, walletBalanceKobo, minPayoutKobo = 100000, transferFeeBufferKobo = 5000 } = $props<{ 
		isOpen: boolean, 
		onClose: () => void,
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
	...
	<div class="relative bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl shadow-lg w-full max-w-md p-6 overflow-hidden">
		...
		<form method="POST" action="?/requestPayout" class="flex flex-col gap-4" use:enhance={() => {
			loading = true;
			...
			<div class="p-3 bg-[var(--bg-element)] border border-[var(--border-color)] rounded-lg flex gap-3">
				<Info size={18} class="text-muted mt-0.5" />
				<div>
					<p class="text-[13px] text-main font-medium m-0 mb-1">Available for Payout: ₦{(maxAvailableKobo / 100).toLocaleString()}</p>
					<p class="text-[12px] text-muted m-0">₦{(transferFeeBufferKobo / 100).toLocaleString()} is reserved for Nomba transfer fees.</p>
				</div>
			</div>

			<div class="flex flex-col gap-1.5 mt-2">
				<label for="amount" class="flex items-center justify-between text-[13px] font-medium text-main">
					<span>Amount (₦)</span>
					<button type="button" class="text-[12px] text-[var(--accent)] bg-transparent border-none p-0 cursor-pointer hover:underline" onclick={setMax}>Max Available</button>
				</label>
				<input 
					type="number" 
					id="amount" 
				...
				/>
				{#if isAmountTooHigh}
					<p class="text-[11px] text-[var(--error-color)] m-0">Amount exceeds maximum available (₦{(maxAvailableKobo / 100).toLocaleString()}).</p>
				{:else if isAmountTooLow}
					<p class="text-[11px] text-[var(--error-color)] m-0">Minimum payout is ₦{(minPayoutKobo / 100).toLocaleString()}.</p>
				{/if}
			</div>
			...
			<div class="mt-4 flex justify-end gap-3">
				...
				<button type="submit" class="h-9 px-4 rounded-lg bg-[var(--accent)] text-[var(--accent-text)] text-[13px] font-semibold border-none cursor-pointer disabled:opacity-50" disabled={loading || !isAmountValid}>
