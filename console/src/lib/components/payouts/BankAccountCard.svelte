	...
	{#if hasBankAccount}
		...
		<div class="flex items-center gap-4">
			...
			<div>
				...
				<p class="text-[13px] text-muted m-0">{bankAccount.accountNumber} • {bankAccount.bankName}</p>
			</div>
		</div>
	{:else}
		<div class="text-center">
			<p class="text-[13px] text-muted mb-3">No bank account configured.</p>
			<button class="text-[13px] font-medium text-[var(--accent)] bg-transparent border-none p-0 cursor-pointer hover:underline" onclick={onConfigure}>
