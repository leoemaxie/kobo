          ...
          <td style="padding: 16px 20px; text-align: right; white-space: nowrap;">
            <div class="flex items-center justify-end gap-2">
              <form method="POST" action="?/changeRole" use:enhance={() => {
                ...
                <button type="submit" class="text-[12px] px-3 py-1.5 rounded-md font-medium text-main bg-element hover:bg-element-hover border border-border transition-colors">
              ...
              <form method="POST" action="?/removeMember" use:enhance={() => {
                ...
                <input type="hidden" name="id" value={member.id} />
                <button type="submit" onclick={(e) => { if(!confirm('Are you sure?')) e.preventDefault(); }} class="text-[12px] px-3 py-1.5 rounded-md font-medium text-red-400 bg-red-400/10 hover:bg-red-400/20 border border-red-500/20 transition-colors">
                  Remove
                </button>
              </form>
            </div>
