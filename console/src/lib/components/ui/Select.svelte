<script lang="ts">
  ...
  let selectedLabel = $derived(
    options.find((o: SelectOption) => o.value === value)?.label || placeholder,
  );
</script>
  ...
  {#if isOpen}
    ...
    >
      {#each options as option}
        <button
          type="button"
          class="w-full text-left px-2.5 py-1.5 rounded-md text-[13px] flex items-center justify-between transition-colors duration-150 {value ===
          option.value
            ? 'bg-[var(--accent)]/10 text-[var(--accent)] font-medium'
            : 'text-main hover:bg-[var(--bg-element)]'}"
