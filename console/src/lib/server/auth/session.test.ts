...
const mockSelectWhere = vi.fn().mockReturnValue({ innerJoin: mockSelectInnerJoin });
...
describe('session.ts', () => {
	...
	describe('validateSession', () => {
		it('should return session data if valid', async () => {
			const mockSessionData = { session: { id: 'sess_1' }, user: { id: 'user_1' } };
			...
			expect(mockSelectInnerJoin).toHaveBeenCalledWith(users, expect.any(Object));
	...
	describe('revokeSession', () => {
		it('should update revokedAt for the specific session', async () => {
			...
			expect(mockUpdateSet).toHaveBeenCalledWith({ revokedAt: expect.any(Date) });
			expect(mockUpdateWhere).toHaveBeenCalledWith(
				expect.objectContaining({ op: 'eq', args: ['sessions.id', 'sess_123'] })
			);
		});
	});
