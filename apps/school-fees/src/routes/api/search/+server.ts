  ...
  try {
    if (locals.user.role === 'admin' || locals.user.role === 'superadmin') {
      ...
      const matchedStudents = await db
        .select()
        .from(students)
        .where(or(ilike(students.name, searchPattern), ilike(students.id, searchPattern)))
        .limit(10);

      const matchedParents = await db
        .select()
        .from(parents)
        .where(or(ilike(parents.name, searchPattern), ilike(parents.email, searchPattern)))
        .limit(10);
      ...
      const matchedStudents = await db
        ...
        .where(
          and(
            or(ilike(students.name, searchPattern), ilike(students.id, searchPattern)),
