...
export const load: PageServerLoad = async ({ params, locals }) => {
    ...
    const linkedLinks = await db.select().from(parentStudents).where(eq(parentStudents.studentId, studentId));
...
export const actions: Actions = {
    closeAccount: async ({ params, locals }) => {
        if (!locals.user || !['admin', 'superadmin'].includes(locals.user.role)) return fail(403, { error: 'Unauthorized' });
    ...
    unlinkParent: async ({ request, params, locals }) => {
        ...
        try {
            await db.delete(parentStudents).where(and(eq(parentStudents.studentId, params.id), eq(parentStudents.parentId, parentId)));
