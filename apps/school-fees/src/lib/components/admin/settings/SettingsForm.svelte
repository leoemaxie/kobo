...
<div class="bg-carbon border border-iron rounded-xl p-8 max-w-2xl">
  <div class="mb-6 space-y-2">
    <div
      class="bg-electric-lime/10 border border-electric-lime/30 text-electric-lime text-sm p-4 rounded-lg"
    >
      <strong>Note:</strong> These credentials are kept in memory and are not saved to the database. They
  ...
  {#if form?.success}
    <div
      class="bg-dark-olive/20 border border-electric-lime/50 text-electric-lime text-sm p-4 rounded-lg mb-6 shadow-sm"
    >
  ...
  <form
    ...
    use:enhance={({ formData, cancel }) => {
      valErr = '';
      const p = String(formData.get('apiKey')),
        s = String(formData.get('apiSecret'));
      const pt = p.startsWith('kobo_test_pk'),
        pl = p.startsWith('kobo_live_pk');
      const st = s.startsWith('kobo_test_sk'),
        sl = s.startsWith('kobo_live_sk');
  ...
  >
    <div class="space-y-1.5">
      <label for="apiKey" class="block text-xs font-semibold text-smoke uppercase tracking-widest"
        >Kobo API Key</label
      >
      <input
        type="text"
