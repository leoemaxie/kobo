...
<div class="bg-carbon border border-iron/50 rounded-xl p-6 shadow-sm">
  ...
  {#if form?.success}
    <div class="bg-dark-olive/20 border border-electric-lime/50 text-electric-lime text-sm p-4 rounded-lg mb-6 shadow-sm transition-all animate-in fade-in slide-in-from-top-2">
      Student registered successfully! Kobo Identity created.
    </div>
  {/if}
  {#if form?.error}
    <div class="bg-danger/10 border border-danger/50 text-danger text-sm p-4 rounded-lg mb-6 shadow-sm transition-all animate-in fade-in slide-in-from-top-2">
      {form.error}
    </div>
  {/if}

  <form method="POST" action="?/register" class="space-y-5" use:enhance={() => {
    isSubmitting = true;
    return async ({ update }) => {
      await update();
      isSubmitting = false;
