  ...
  <div class="grid grid-cols-1 gap-4">
    {#each students as student}
      <a href="/admin/students/{student.id}" class="block group">
        <div
          class="bg-carbon border border-iron/50 group-hover:border-electric-lime/30 rounded-xl p-5 transition-all duration-300 group-hover:-translate-y-0.5 shadow-sm group-hover:shadow-electric-lime/5 flex flex-col md:flex-row md:items-center justify-between gap-6"
        >
          <div class="flex items-center gap-4">
            <div
              class="w-12 h-12 shrink-0 rounded-full bg-void-black border border-iron flex items-center justify-center text-sm font-bold text-electric-lime tracking-wider group-hover:border-electric-lime/50 transition-colors"
            >
              {getInitials(student.name)}
            </div>
            <div>
              <div
                class="text-sm font-medium text-pure-white group-hover:text-electric-lime transition-colors"
              >
                {student.name}
              </div>
              <div class="text-xs text-smoke mt-1">
                Class: {student.class} &bull; Reg: {student.date}
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 flex-1 justify-end">
            <div class="hidden lg:block text-right">
              <div class="text-[10px] font-bold uppercase tracking-widest text-smoke mb-1">
                Parents
              </div>
              ...
              {#if student.virtualAccountNo}
                <div class="flex items-center md:justify-end gap-2">
                  ...
                  <button
                    ...
                    onclick={(e) => {
                      e.preventDefault();
                      copyToClipboard(student.virtualAccountNo!, student.id);
                    }}
            ...
            <div
              class="shrink-0 text-smoke group-hover:text-electric-lime transition-colors hidden md:block"
            >
