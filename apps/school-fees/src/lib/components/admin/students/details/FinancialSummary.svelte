...
<div class="bg-carbon border border-iron rounded-xl p-6 h-full shadow-sm flex flex-col gap-8">
  <div>
    <h3 class="text-sm font-semibold text-smoke uppercase tracking-widest mb-4">
      Financial Summary
    </h3>

    <div class="">
      <div class="text-xs text-smoke mb-1">Current Balance</div>
      <div class="text-4xl font-bold text-pure-white mt-2">
        {formatCurrency(statement.balanceKobo)}
      </div>
    </div>
  </div>
      ...
      <div class="space-y-3">
        <div>
          ...
          <div class="flex items-center gap-2 mt-0.5">
            ...
            <button
              onclick={copyAccount}
              class="text-smoke hover:text-electric-lime focus:outline-none transition-colors"
            >
              {#if copied}
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div class="text-sm font-medium text-pure-white mt-0.5 truncate">
              {account.accountName}
            </div>
          </div>
        </div>
