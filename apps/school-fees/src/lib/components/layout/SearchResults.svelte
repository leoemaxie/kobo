...
<div class="overflow-y-auto flex-1 p-2 custom-scrollbar">
  {#if results}
    ...
    {:else}
        ...
        <ul class="mb-2">
          {#each results.students as student}
            <li>
              ...
              >
                <div>
                  <div class="font-medium text-pure-white group-hover:text-electric-lime transition-colors">
                    {student.name}
                  </div>
                ...
                <div class="text-xs text-smoke font-mono opacity-0 group-hover:opacity-100 transition-opacity">
                  ↵ Enter
                </div>
              </button>
            </li>
      ...
      {#if results.parents.length > 0}
        <div class="px-3 py-2 text-xs font-bold text-smoke uppercase tracking-widest mt-2 border-t border-iron/50 pt-4">
          Parents
        </div>
        <ul>
            ...
            <li>
              <div class="px-4 py-3 rounded-xl flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-full bg-void-black border border-iron flex items-center justify-center text-smoke">
                    <User size={14} />
                  </div>
                  <div>
  ...
  {:else if query.length > 0 && query.length < 2}
    <div class="py-8 text-center text-smoke text-sm">
      Type at least 2 characters to search...
    </div>
  {:else}
    <div class="py-8 px-6 flex items-center justify-center gap-4 text-smoke text-xs hidden sm:flex">
      <div class="flex items-center gap-1">
        <kbd class="bg-void-black border border-iron px-1.5 py-0.5 rounded shadow-sm font-mono font-bold text-[10px]">↑</kbd>
        <kbd class="bg-void-black border border-iron px-1.5 py-0.5 rounded shadow-sm font-mono font-bold text-[10px]">↓</kbd>
      ...
      <div class="flex items-center gap-1">
        <kbd class="bg-void-black border border-iron px-1.5 py-0.5 rounded shadow-sm font-mono font-bold text-[10px]">ESC</kbd>
